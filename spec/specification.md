...
retrieve the [[ref: DID Log]]. This process described here addresses the handling of [international domain names](#international-domain-names).
...
3. **Handle the domain name**:
   - If the domain contains a port, decode percent-encoding and preserve the port.
   - If the domain contains non-ASCII characters, apply IDNA (Punycode) encoding per [[spec:rfc3492]].
4. **Convert path segments**:
   - Each path segment must be percent-encoded per [[spec:rfc3986]] to ensure a valid HTTP URL.
